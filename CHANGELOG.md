...
- **A reveal toggle on the password fields** of the sign-in and sign-up forms. This password is the encryption key and nobody can reset it, so being able to check what was typed is the cheapest guard against locking yourself out of your own vaults.
...
- **Every other signed-in device is signed out by a password change.** Tokens now carry the key generation they were issued under; a session from before the change is refused on any write and on token refresh — instead of silently saving titles and uploads under keys that no longer exist — and signs back in with the new password. Sessions from before this release get the same treatment once their access token expires.
- **An interrupted password change tells you the truth.** If the connection dies mid-change, the app checks which Argon2 salt the server is handing out — the change always mints a fresh one — and reports definitively whether the new password took, instead of guessing.
- `GET /api/auth/rotation-manifest` — one snapshot of everything a rotation must rewrite; `POST /api/auth/rotate-credentials` now takes (and requires) the re-wrapped attachment keys. The server refuses any bundle that would leave a title, note or attachment under the old key.
- Tests: `tests/endpoints/password-rotation.mjs` (the contract — coverage, atomicity, stale sessions, the salt probe) and `tests/ui/password-rotation.mjs` (the real client crypto in a browser, legacy-wrap upgrade included).
...
### Security
- What a password change does **not** do, stated so nobody assumes otherwise: it does not rotate the key-pair (a compromised key-pair is not healed by a new password), and there is still no password reset — the server holds nothing that could recreate the keys. The Account screen says so in the place someone goes looking for the reset button.

### Fixed
- **The password-rotation transaction was not isolated.** The backend runs every handler on one shared PostgreSQL connection, so the rotation's `BEGIN`/`COMMIT` could sweep a concurrent, unrelated write into its transaction — and roll it back with a failed rotation. Rotation now runs on its own dedicated connection, with the version check and completeness checks inside the transaction under a row lock, rather than before it.
...
- **The recommended install pulled a stale, amd64-only image.** `scripts/publish_dockerhub/setup.sh` and the default `docker-compose.yml` both pull from Docker Hub, but Docker Hub publishing was removed from the build workflow in May, so everything since went to GHCR alone. Docker Hub's `latest` stopped at v0.3.30 (7 June) and never gained the arm64 half, so a Raspberry Pi, Ampere VPS or Apple Silicon Mac following the documented install failed outright with `no matching manifest for linux/arm64`. Releases now publish the same digests to both registries; a fork without the Docker Hub secrets still publishes to GHCR alone rather than failing. `docs/DEPLOYMENT.md` documents both registries, what each tag means, and how to republish a release to a registry it did not reach.
